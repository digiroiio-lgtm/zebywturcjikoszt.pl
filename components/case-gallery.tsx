...
export function CuratedCaseGallery() {
  ...
  return <section className="shell section-space visual-proof" aria-labelledby="visual-proof-title">
    ...
    <div className="visual-proof-actions"><Link className="text-link" href="/przed-i-po">Zobacz więcej zdjęć <span aria-hidden="true">→</span></Link><TrackedLink href={assessmentHref("home_visual_proof", "/")} event="home_visual_assessment_cta" tracking={{ lead_source: "OGZ-PL", cta_location: "home_visual_proof" }} className="button">Poproś o wstępną ocenę</TrackedLink></div>
  </section>;
}
...
export function BeforeAfterCaseHub({ formEnabled }: { formEnabled: boolean }) {
  return <section className="case-hub" aria-labelledby="case-hub-title">
    <div className="case-hub-heading"><p className="eyebrow">Zdjęcia przed i po</p><h2 id="case-hub-title">Przykładowe metamorfozy</h2><p>Rezultaty różnią się między pacjentami. Zdjęcia nie pozwalają ustalić kwalifikacji do leczenia; potrzebne są diagnostyka i konsultacja.</p></div>
    ...
    <InPageLeadCta title="Chcesz poznać możliwy zakres leczenia i elementy wyceny?" text="Krótki opis problemu pomoże ustalić, o co warto zapytać podczas konsultacji." buttonLabel="Poproś o wstępną ocenę" location="before_after_cases_5_10" caseReference="before-after10" formEnabled={formEnabled} />
