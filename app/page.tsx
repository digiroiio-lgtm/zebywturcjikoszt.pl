...
export const metadata: Metadata = {
  title: "Zęby w Turcji – koszt, ceny i leczenie w Antalyi",
  description: "Zęby w Turcji bez marketingowych skrótów. Sprawdź koszty, implanty, licówki, pełną odbudowę, opinie i plan leczenia w Antalyi.",
  alternates: { canonical: "/" }
};

const treatments = [
  { href: "/koszt/", label: "Koszt leczenia", text: "Co wpływa na cenę i jak porównać pełny zakres wyceny." },
  { href: "/implanty/", label: "Implanty", text: "Kwalifikacja, etapy, odbudowa i pytania do kliniki." },
  { href: "/licowki/", label: "Licówki", text: "Plan estetyczny, materiały i alternatywy dla licówek." },
  { href: "/cala-szczeka/", label: "Cała szczęka", text: "Dlaczego pełna odbudowa nie oznacza jednego zabiegu." },
  { href: "/all-on-4/", label: "All-on-4", text: "Na czym polega koncepcja i co decyduje o kwalifikacji." },
  { href: "/antalya/", label: "Leczenie w Antalyi", text: "Jak przygotować wyjazd, wizyty i opiekę po powrocie." }
];

export default function Home() {
  return <main>
    <section className="home-hero"><div className="shell hero-grid"><div><p className="eyebrow">Przewodnik dla pacjentów z Polski</p><h1>Zęby w Turcji – koszt leczenia i możliwości</h1><p className="hero-lead">Sprawdź, jak porównać leczenie w Antalyi, zrozumieć wycenę i przygotować pytania przed podjęciem decyzji.</p><div className="button-row"><TrackedLink href="/kontakt/" event="home_assessment_cta" className="button">Sprawdź koszt swojego leczenia</TrackedLink><TrackedLink href="/jak-wybrac-klinike/" event="home_clinic_guide_cta" className="text-link">Jak wybrać klinikę <span aria-hidden="true">→</span></TrackedLink></div><p className="microcopy">Bez fikcyjnych cen, opinii i obietnic rezultatu. Formularz zostanie aktywowany po potwierdzeniu bezpiecznego procesu kontaktu.</p></div><div className="hero-panel"><p className="mini-label">Zacznij od swojej potrzeby</p><ol><li><span>01</span><div><strong>Porównaj możliwości</strong><p>Implanty, licówki i pełna odbudowa mają inne wskazania.</p></div></li><li><span>02</span><div><strong>Zrozum pełny koszt</strong><p>Zakres, materiały, etapy, podróż i opieka po leczeniu.</p></div></li><li><span>03</span><div><strong>Poproś o indywidualny plan</strong><p>Ostateczna decyzja wymaga diagnostyki i konsultacji.</p></div></li></ol></div></div></section>
    <section className="shell section-space"><div className="section-heading"><div><p className="eyebrow">Ścieżki leczenia</p><h2>Znajdź właściwy punkt startu</h2></div><p>Każda strona odpowiada na inny dominujący zamiar wyszukiwania. Koszt, metoda leczenia i wybór kliniki nie są mieszane w jedną obietnicę.</p></div><div className="treatment-grid">{treatments.map((item, index) => <Link className="treatment-card" href={item.href} key={item.href}><span>0{index + 1}</span><h3>{item.label}</h3><p>{item.text}</p><b>Przeczytaj przewodnik →</b></Link>)}</div></section>
    ...
    <section className="shell editorial-strip"><p><strong>Autor:</strong> Redakcja serwisu</p><p><strong>Ostatnia aktualizacja:</strong> {UPDATED_DATE}</p><p><strong>Recenzja medyczna:</strong> jeszcze nieprzeprowadzona</p><Link href="/polityka-redakcyjna/">Standard redakcyjny</Link></section>
    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "WebPage", name: "Zęby w Turcji – koszt leczenia i możliwości", url: SITE_URL, inLanguage: "pl-PL", dateModified: "2026-09-18" }) }} />
  </main>;
}
