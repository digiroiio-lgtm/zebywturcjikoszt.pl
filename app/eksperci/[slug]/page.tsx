...
export default async function ExpertProfile({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="shell narrow content-section">
    ...
    <section><h2>Zweryfikowane treści</h2>{reviewedPages.length > 0 ? <ul>{reviewedPages.map((page) => <li key={page.slug}><Link href={`/${page.slug}`}>{page.title}</Link></li>)}</ul> : <p>Przypisanie recenzji do konkretnych stron oczekuje na potwierdzenie daty przeglądu. Profil zawodowy nie jest dowodem recenzji artykułu.</p>}</section>
